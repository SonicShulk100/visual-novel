label chapter_2_part_1:
    scene forest-day
    with fade

    "The morning sun has risen, with its glow filtering through the dense canopy of the forest."

    "Birds chirp melodiously, creating a symphony that resonates with the rustling leaves."

    if dijo_picked:
        show dijo neutral at center
        with dissolve

        "Dijonay was the first to wake up, stretching and yawning as she took in the beauty of the morning."

        "She noticed Kurt still asleep with his head resting on her breasts."

        dijo "(Seems that Kurt is still asleep... I should let him rest a bit longer.)"

        "Dijonay carefully adjusted her position to avoid disturbing him, a gentle smile on her face."

        dijo "(I wonder what adventures await us today in this enchanting forest.)"

        "She then stood up with her strength and, began groping her ass to make sure that she is still awake."

        dijo "Hmmm.... That feels so good..."

        show mari neutral at left
        show jess neutral at right
        with dissolve

        dijo "Ah... Good morning, you two."

        mari "Good morning, Dijo."

        jess "Good morning, Dijo..."

        dijo "Are Cindy and Jehanne still asleep?"

        jess "They must be tired after what happened last night."

        dijo "I can tell..."

        hide mari neutral
        show cin neutral at left
        with dissolve

        cin "Good morning, you three..."

        dijo "Oh, Cindy! You're awake too."

        hide jess neutral
        show jeha neutral at right
        with dissolve

        jeha "Good morning, everyone..."

        dijo "I guess everyone, except for Kurt, is awake."

        cin "Who's Kurt...?"

        dijo "The boy's name who's still asleep."

        dijo "Let's prepare to leave the forest soon."

        jump chapter_2_part_2

    elif mari_picked:
        show mari neutral at center
        with dissolve

        "Maria was the first to wake up, her eyes fluttering open as she took in the serene surroundings."

        "She noticed Kurt still asleep with his head resting on her breasts."

        mari "(Seems that Kurt is still asleep... I should let him rest a bit longer.)"

        "Maria carefully adjusted her position to avoid disturbing him, a gentle smile on her face."

        mari "(I wonder what adventures await us today in this enchanting forest.)"

        show jess neutral at left
        with dissolve

        jess "Good morning, Maria."

        mari "Good morning, Jessica."

        jess "Are the others still asleep?"

        mari "Yep. Seems I'm the first to wake up and you would be the second."

        jess "To think that we all fell asleep so deeply last night..."

        show dijo neutral at right
        with dissolve

        dijo "Good morning, you two."

        mari "Oh, Dijo! You're awake too."

        jess "Good morning, Dijo."

        hide dijo neutral
        hide jess neutral
        show cin neutral at left
        show jeha neutral at right
        with dissolve 

        cin "Good morning, everyone..."

        jeha "Good morning, ladies~"

        mari "I guess everyone, except for Kurt, is awake."

        cin "Who are you talking about?"

        mari "That boy's name is Kurt. He's still asleep."

        hide jeha neutral
        show jess neutral at right
        with dissolve

        jess "I see... Should we wake him up soon?"

        mari "Not yet... Let him rest a bit longer."

        jump chapter_2_part_2

    elif jess_picked:
        show jess neutral at center
        with dissolve

        "Jessica was the first to wake up, her eyes fluttering open as she took in the serene surroundings."

        show mari neutral at left
        with dissolve

        "She noticed Maria waking up as well, yawning in the process."

        jess "Good morning, Maria."

        mari "Good morning, Jessica."

        mari "Is Kurt still asleep?"

        jess "Yes, he is. We should let him rest a bit longer."

        mari "I wonder if the others are still asleep..."

        jess "Probably. This forest is so peaceful; it's hard to wake up early here."

        show dijo neutral at right
        with dissolve

        dijo "Good morning, you two."

        mari "Oh, Dijo! You're awake too."

        jess "Good morning, Dijo."

        hide dijo neutral
        hide mari neutral
        show cin neutral at left
        show jeha neutral at right
        with dissolve

        cin "Good morning, everyone..."
        jeha "Good morning, ladies~"

        jess "I guess everyone, except for Kurt, is awake."

        hide jeha neutral
        show dijo neutral at right
        with dissolve

        dijo "Who's Kurt...?"

        jess "He's the boy's name who's still asleep."

        jump chapter_2_part_2
    
    elif cin_picked:
        show cin neutral at center
        with dissolve

        "Cindy was the first to wake up, her eyes fluttering open as she took in the serene surroundings."

        cin "Oh geez... I overslept again..."

        "She noticed Kurt still asleep with his head resting on her breasts."

        cin "(At least Kurt is still asleep... I should let him rest a bit longer.)"

        "She noticed the boy waking up."

        kurt "Mmm... Where am I...?"

        cin "Shh... It's okay, Kurt. You're safe."

        kurt "Are you still craddling me...?"

        cin "Of course I am. You feel safe in my arms."

        cin "Um... Please don't tell the others that you woke up, okay...?"

        kurt "Alright... I won't say anything."

        "Kurt then went back to sleep."

        "There..."

        show mari neutral at right
        show jess neutral at left
        with dissolve

        mari "Good morning, Cindy."
        jess "Good morning, Cindy."

        cin "Good morning, you two."

        jump chapter_2_part_2
    
    elif jeha_picked:
        show jeha neutral at center
        with dissolve

        "Jehanne was the first to wake up, her eyes fluttering open as she took in the serene surroundings."

        "Before she began to move, she slapped her ass and rubbed her pussy a little bit through her purple panties."

        show dijo neutral at right
        with dissolve

        "There, she noticed Dijonay awake, still craddling the sleeping Kurt in her arms."

        jeha "Good morning, Dijo..."

        dijo "Good morning, Jehanne."

        jeha "That kid is still asleep...?"

        dijo "Yes, he is. We should let him rest a bit longer."

        show mari neutral at left
        with dissolve

        mari "Good morning, Jehanne and Dijo."

        hide jeha neutral
        show jess neutral at center
        with dissolve

        jess "Good morning, everyone."

        hide dijo neutral
        show cin neutral at right
        with dissolve

        cin "Yawn... Good morning, everyone..."

        jump chapter_2_part_2